...
After computing number of given word in the book, we can finally compute to probability. For example purposes, lets say we want the word `that`. So we will search the word, divide it in the total number of word in book, and because we hate two independant experiments we will raise it to the power of 2. We will receive ```r (Q3a("that")/total_words_amount)^2`.
