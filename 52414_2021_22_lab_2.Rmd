...
Our initial `full_book_string` have `r length(full_book_string)` words, and our `full_unique_book_string` have `r length(full_unique_book_string)` words. Now let us check everything, starting with the wanted function for a specific word.
...
Just like in the dice example, when we have unique numbers all the time, we can do the same here, and compute it as one over length of `unique_full_book`. the result is `r (1/length(unique_full_book))^2`. the probability is extremely smaller than the others. So yeah, it is smaller.
